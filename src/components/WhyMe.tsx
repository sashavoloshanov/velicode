...
import { MessageCircle, Shield, Layers, ClipboardCheck, Headphones, Rocket } from "lucide-react";

const icons = [
  <MessageCircle key="mc" size={24} />,
  <Shield key="sh" size={24} />,
  <Layers key="ly" size={24} />,
  <ClipboardCheck key="cc" size={24} />,
  <Headphones key="hp" size={24} />,
  <Rocket key="rk" size={24} />,
];
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {t.why.items.map((item, i) => (
            <div key={i} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6">
              <span className="inline-flex rounded-xl bg-[var(--color-bg-secondary)] p-3 text-[var(--color-accent)]">{icons[i % icons.length]}</span>
